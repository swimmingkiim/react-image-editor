...
const useTab = (
  ...
  const onCreateTab = (
    ...
    const newTabId = fileItem?.id
            ?? `file-${
              tabList.length === 0
                ? 1
                : parseInt(tabList[tabList.length - 1].id.slice(5)) + 1
            }`;
  ...
  const onDeleteTab = (tabId: string) => {
    ...
    const nextTabId = tabList[tabIndex].id === currentTab!.id
