...
const useDragAndDrop = (
        ...
        const newText: StageData = {
            ...
            attrs: {
                ...
                width: data.text
                    .split('')
                    .reduce(
                        (acc: number, curr: string) => (curr.charCodeAt(0) >= 32 && curr.charCodeAt(0) <= 126
    ...
    const insertLine = (e: DragEvent, data: { [key: string]: any }) => {
        const position = getFramePos(stageRef.current, e, 100, 100);
        const curvePoints = data.name.indexOf('curve') !== -1
    ...
    const moveToLayer = (item: Shape<ShapeConfig>) => {
        ...
        item.getParent().children = (item.getParent().children as Node<NodeConfig>[])?.filter(
